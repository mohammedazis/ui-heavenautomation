<script lang="ts">
	...
	import { ListInstallations } from '$lib/widgets';
</script>

<HeadTitle title="Installations" />

<div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto relative">
	<Breadcrumb title="Installations" pagetitle="Heaven Automation" />
	<div class="card">
		<div class="card-body">
			<div class="flex items-center gap-3 mb-4">
				<h4 class="text-15 grow">Installations</h4>
				...
				{/if}
				{#if hasPermission(data.authUser.permissions, PermissionTypes.Create, Modules.Installations)}
					<div class="shrink-0">
						<a
							href="/installations/add"
							class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20"
							><LucideIcon name="Plus" class="inline-block size-4" />
							<span class="align-middle">Add Installation</span></a
						>
					</div>
				{/if}
			</div>
