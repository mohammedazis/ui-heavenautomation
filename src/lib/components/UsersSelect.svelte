<script lang="ts">
	...
	import type { ServiceCall, User } from '$lib/types';
	...
	if (selectedServiceCall.assignedToSecond) {
		secondUserId = selectedServiceCall.assignedToSecond._id;
		secondValue = selectedServiceCall.assignedToSecond.name;
	}

	if (selectedServiceCall.assignedTo) {
		userId = selectedServiceCall.assignedTo._id;
		value = selectedServiceCall.assignedTo.name;
	}
	...
	<div class="max-h-[calc(theme('height.screen')_-_180px)] p-4">
		<div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
			<div class="xl:col-span-12 relative">
				<form
					action="/service-calls?/assignPeople"
					method="POST"
					use:enhance={() => {
						return async ({ result }) => {
							updateFlash(page);
				...
				>
					...
					<div class="flex items-center">
						<input
							type="text"
							bind:value={selectedServiceCall._id}
							id="serviceId"
							name="serviceId"
